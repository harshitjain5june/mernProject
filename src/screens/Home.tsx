...
function Home() {
  ...
  const fetchData = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_BASE_URL}/api/displayFoodItems`,
      {
        method: "GET",
        headers: {
    ...
    <>
      ...
      <div className="container">
        {foodCategory?.map((item: any) => (
          <>
            <div style={{ marginTop: "15px" }} key={item._id}>
              <h3 style={{ paddingLeft: "12px", marginBottom: "0px" }}>
                {item.CategoryName}
              </h3>
              <div className="menu">
                {foodItems
                  .filter(
                    (foodData: any) =>
                      foodData.CategoryName === item.CategoryName,
                  )
                  .map((subItem: any) => (
                    <>
                      {console.log("this is our subitem", subItem)}
                      <Card foodItem={subItem} options={subItem.options[0]} />
                    </>
                  ))}
              </div>
            </div>
